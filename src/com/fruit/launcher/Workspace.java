...
public class Workspace extends ViewGroup implements DropTarget, DragSource,
	...
	private void updateWallpaperOffset(int scrollRange) {
		...
		if (token != null) {
			...
			Log.i(TAG, "updateWallpaperOffset, mWallpaperIndex="
			...
			if (tempScrollX > (float) scrollRange) {
				...
				int alpha = 255 - (int) (((tempScrollX - scrollRange) * 1.0
						/ getWidth() * 1.0) * 255);
				Log.i(TAG, "updateWallpaperOffset, alpha=" + alpha
	...
	private int getYPos(int index, int thumbHeight) {
		...
		int pos = thumbHeight * (index / mRowCount) + mHeightStatusBar
				+ offsetY + mHeightStartPadding;// + 12;//39; //magic number :
												// 200 - 39 = 161;
		Log.i(TAG, "moveitem,ypos=" + pos + ",mHeightStatusBar="
				+ mHeightStatusBar);
		return pos;
	...
	void moveItem(int init, int offset, View child, CellLayout current) {
		...
		Log.i(TAG, "moveItem,(x1,y1)=(" + x1 + "," + y1 + "),(x2,y2)=(" + x2
				+ "," + y2 + ")");
	...
	public void setLongClickValues(CellLayout.CellInfo cellInfo) {
		...
		Log.i(TAG, "moveitem,getMeasuredWidth=" + getMeasuredWidth()
				+ ", getMeasuredHeight=" + getMeasuredHeight());
		Log.i(TAG, "moveitem,getLeft=" + getLeft() + ", getRight=" + getRight()
				+ ",getTop=" + getTop() + ", getBottom=" + getBottom());
		Log.i(TAG, "moveitem,getWidth()=" + getWidth() + ", getHeight()="
				+ getHeight());
		...
		Log.i(TAG, "moveitem,Width()=" + Launcher.mScreenWidth + ", Height()="
		...
		Log.i(TAG, "moveitem,mCellWidth=" + mCellWidth + ", mCellHeight="
				+ mCellHeight);
...
//		}
		...
		if (source != this) {
			...
			if (mDragInfo != null && mDragInfo.cell != null) {
				...
				if (isViewSpan1x1(mDragInfo)) {
					...
					// cellLayout.pointToCellRounded(x - xOffset, y - yOffset,
					// newCell);
					//
					// if (!(isViewMoveToSamePlace(mDragInfo, mDragInfo.screen,
					// index, newCell[0], newCell[1]))) {
					// //cellLayout.checkCellLayout();
					// int dropIndex = cellLayout.cellToIndex(newCell[0],
					// newCell[1]);
					// if (dropIndex >= 0) {
					// if ((cellLayout.getCellSpanX(dropIndex) == 1) &&
					// (cellLayout.getCellSpanY(dropIndex) == 1)) {
					// View dropView = cellLayout.getChildAt(dropIndex);
					// if (!(dropView instanceof Folder)) {
					// //onDropScreen(mDragInfo, mDragInfo.screen, index,
					// newCell[0], newCell[1]);
					// onDropInternalEx(x, y , xOffset, yOffset, cell,
					// cellLayout, index);
					// exchangeAllCells(index);
					// return;
					// }
					...
					if (cellLayout.getPageIndex() != itemInfo.screen) {
						if (cellLayout.isFull()
								|| !acceptDropInternal(source, x, y, xOffset,
										yOffset, dragView, dragInfo)) {
							// cellLayout.removeView(mDragInfo.cell);
							// oriLayout.addView(mDragInfo.cell);
							// final CellLayout originalCellLayout =
							// (CellLayout) getChildAt(itemInfo.screen);
	...
	public void onDragEnter(DragSource source, int x, int y, int xOffset,
		...
		Log.i(TAG, "917, params(x,y) = " + params.x + "," + params.y
	...
	public void onDragOver(DragSource source, int x, int y, int xOffset,
		...
		if (mIsOverFinished) {
			...
			if (lastLayout.getPageIndex() != current.getPageIndex()){//page changed
				lastLayout.removeView(mDragInfo.cell);
				exchangeAllCells(this.getChildIndexByPos(lastLayout.getPageIndex()));
				current.addView(mDragInfo.cell);
				lastLayout = current;				
			}			
			
			if (current.isOverFull()
					/*&& (source != this || (current.indexOfChild(mDragInfo.cell) < 0))
					|| !isViewSpan1x1(mDragInfo)*/) {
				// Toast.makeText(getContext(),
				// getContext().getString(R.string.out_of_space),
			...
			Log.i(TAG, "914,(x,y)=" + x + "," + y + "cell(" + overCell[0] + ","
			...
			if (newIndex >= 0){
				if(isViewSpan1x1(current.getChildAt(newIndex))) {
					if (!current.getChildAt(newIndex).equals(mDragInfo.cell)) {
						Log.i(TAG, "914,overCell is not empty,move the item in it "
								+ mFromPos + "-" + mToPos);
	...
	public int getSeqNo(int screen, int cellX, int cellY){
		return screen*(ItemInfo.ROW*ItemInfo.COL)+cellY*(ItemInfo.COL)+cellX;
	}
	...
	public void printChildCount() {
		Log.i(TAG, "workspace default screen is " + this.mDefaultScreen
				+ ", current screen is " + this.mCurrentScreen);
		for (int i = 0; i < this.getChildCount(); i++) {
			CellLayout layout = (CellLayout) this.getChildAt(i);
			Log.i(TAG, "workspace has " + this.getChildCount() + "," + i
